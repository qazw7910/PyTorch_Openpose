...
class main :
    ...
    def start(self):
        ...
        try:
            ...
            time_step = 0
            num_keypoint = 25
            set_for_model_frame_cnt = 10
            feature_array = np.zeros([set_for_model_frame_cnt, num_keypoint, 3], np.float32)
            ...
            while True:
                ...
                if event == 'Exit' or event == sg.WIN_CLOSED:
                    ...
                    imgbytes = cv2.imencode('.png', op_frame)[1].tobytes()  # this is faster, shorter and needs less includes
                    ...
                    if len(buffer) >= self.num_gpus:
                        ...
                        for image_base_id in range(0, len(buffer), self.num_gpus):
                            ...
                            for gpuId in range(0, self.num_gpus):
                                if time_step < set_for_model_frame_cnt:
                                    datum = op.Datum()
                                ...
                                else:   # delivery norm_keypoints_buffer to model recognize and return body state

                                    # normalize the 25(x,y) keypoint
                                    norm_features = normalization(feature_array, (video_info['W'], video_info['H']))

                                    # select norm_features keypoint and reshape
                            ...
                            for gpuId in range(0, self.num_gpus):
                                ...
                                if image_id < len(buffer):
                                    ...
                                    op_frame = cv2.resize(src=op_frame, dsize=(0,0), fx=1, fy=1, interpolation=cv2.INTER_AREA)
                                    imgbytes = cv2.imencode('.png', op_frame)[1].tobytes()
