...
def train_model(model, train_loader, val_loader, test_loader, epoch):  # test loader
    ...
    criterion = torch.nn.CrossEntropyLoss()  # https://zhuanlan.zhihu.com/p/98785902
    ...
    for phase in ['train', 'val', 'test']:
        plt.plot(range(len(loss_history[phase])), loss_history[phase], label=phase)
    plt.legend()
    ...
    plt.clf()
    for phase in ['train', 'val', 'test']:
        plt.plot(range(len(acc_history[phase])), acc_history[phase], label=phase)
    plt.legend()
...
time_steps = 15
train_dataset = BodyKeypointsDataset(keypoints, root_dir='data_classification/train_dataset', timesteps=time_steps,
                                     transforms={'train': train_transforms}, pad_by_last=True)
val_dataset = BodyKeypointsDataset(keypoints, root_dir='data_classification/val_dataset', timesteps=time_steps,
                                   transforms={'val': val_transforms}, pad_by_last=True)
test_dataset = BodyKeypointsDataset(keypoints, root_dir='data_classification/test_dataset', timesteps=time_steps,
                                    transforms={'val': test_transforms}, pad_by_last=True)
...
torch.save(model.state_dict(), 'pt_model/fall_10_fps.pth')
